...
const StudentSignup = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: "",
    confirmPassword: "",
    gender: "MALE", // ✅ default gender to avoid backend 400
  });
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case "gender":
        if (!value) message = "Gender is required."; // ✅ fixed validation
        else isValid = true;
        break;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const signupData = {
        ...
        phone: formData.phone,
        password: formData.password,
        gender: formData.gender, // ✅ send default or selected gender
      };
      ...
      if (err.response?.status === 400 || err.response?.status === 409) {
        ...
        if (backendMsg.toLowerCase().includes("email")) {
          ...
          setApiError("Phone number already exists. Please use another number.");
        } else if (backendMsg.toLowerCase().includes("gender")) {
          setApiError("Gender is required. Please select one.");
        } else {
          setApiError(backendMsg);
        }
      } else if (err.code === 'ERR_NETWORK') {
